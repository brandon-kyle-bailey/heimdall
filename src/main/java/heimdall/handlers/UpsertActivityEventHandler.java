...
public class UpsertActivityEventHandler implements IEventHandler {
  ...
  public Object handle(JSONObject event) {
    ...
    try {
      event.put("id", 0);
      ActivityDto activity = ActivityDtoMapper.interfaceToDomain(event);

      List<ActivityDto> foundActivities = repository.findLatestByNameTitleUrl(activity.getName(),
          activity.getTitle(),
          activity.getUrl());
      ActivityDto foundActivity = foundActivities.isEmpty() ? null : foundActivities.get(0);
      ...
      LoggerPort.error("SQL Error during UpsertActivityEventHandler.handle: %s".formatted(e.getMessage()));
      e.printStackTrace();
    } catch (Exception e) {
      LoggerPort.error("Unexpected error during UpsertActivityEventHandler.handle: %s".formatted(e.getMessage()));
      e.printStackTrace();
    }
