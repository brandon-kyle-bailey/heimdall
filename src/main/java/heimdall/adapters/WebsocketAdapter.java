...
public class WebsocketAdapter extends WebSocketServer {
  ...
  public WebsocketAdapter(int port, EventbusAdapter eventbus, String[] channels) {
    ...
    for (String channel : channels) {
      ...
      LoggerPort.debug("Initialized channel: %s".formatted(channel));
    }
  }

  @Override
  public void onOpen(WebSocket conn, ClientHandshake handshake) {
    LoggerPort.debug("WebSocket connection opened: %s".formatted(conn.getRemoteSocketAddress()));
  }

  @Override
  public void onClose(WebSocket conn, int code, String reason, boolean remote) {
    LoggerPort.debug("WebSocket connection closed: %s".formatted(conn.getRemoteSocketAddress()));
    channelSubscriptions.forEach((channel, subscribers) -> unsubscribeFromChannel(conn, channel));
  }

  @Override
  public void onMessage(WebSocket conn, String message) {
    LoggerPort.debug("Received WebSocket message: %s".formatted(message));
    try {
      ...
      switch (action) {
        ...
        default:
          LoggerPort.warn("Unrecognized action: %s".formatted(action));
          conn.send("Error: Unrecognized action");
          break;
      }
    } catch (Exception e) {
      LoggerPort.error("Failed to handle message: %s".formatted(message));
  ...
  public void onError(WebSocket conn, Exception ex) {
    LoggerPort.error("WebSocket error: %s".formatted(ex.getMessage()));
    ex.printStackTrace();
  }

  @Override
  public void onStart() {
    LoggerPort.debug("WebSocket server started.");
  }
  ...
  private void subscribeToChannel(WebSocket conn, String channel) {
    ...
    LoggerPort.debug("Connection %s subscribed to channel %s".formatted(conn.getRemoteSocketAddress(), channel));
  }
  ...
  private void unsubscribeFromChannel(WebSocket conn, String channel) {
    ...
    LoggerPort.debug("Connection %s unsubscribed from channel %s".formatted(conn.getRemoteSocketAddress(), channel));
  }
  ...
  public String broadcastToChannelSync(String channel, String message) {
    ...
    CompletableFuture<String>[] futures = subscribers.stream()
        .map(conn -> CompletableFuture.supplyAsync(() -> {
          try {
            ...
            LoggerPort.error("Failed to send message to %s".formatted(conn.getRemoteSocketAddress()));
    ...
    try {
      ...
      LoggerPort.error("Error waiting for broadcast responses: %s".formatted(e.getMessage()));
    }
  ...
  public void addDefaultSubscriber(String channel, Consumer<String> subscriber) {
    if (!channelSubscriptions.containsKey(channel)) {
      LoggerPort.warn("Channel %s does not exist, initializing it.".formatted(channel));
      channelSubscriptions.putIfAbsent(channel, new CopyOnWriteArraySet<>());
    }
    defaultSubscribers.put(channel, subscriber);
    LoggerPort.debug("Added default subscriber for channel: %s".formatted(channel));
  }
}
